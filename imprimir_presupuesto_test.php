<?php 
	...
	if($id!=0){
		...
		$statement = $pdo->prepare("SELECT t1.*, t2.*, t3.p_name, t3.p_code, t4.*, FROM_UNIXTIME(t2.fecha, '%d/%m/%Y %H:%i') fecha_format FROM `detalle` AS t1 INNER JOIN presupuesto AS t2 ON t2.id_presupuesto = t1.id_presupuesto INNER JOIN tbl_product AS t3 ON t1.id_producto = t3.p_id INNER JOIN tbl_cliente as t4 ON t2.id_cliente = t4.c_id WHERE t2.id_presupuesto = ? ;");
		...
		$(document).ready(function(){
			...
			var dd = {
				...
				content: [
					...
					{
					    text: '<?php echo $c_calle." N° ".$c_calle_nro." - ".$c_barrio; ?>',
					    style: 'valor',
								...
								[
								 ...
								 <?php 
									...
									foreach($resultado as $row){
										...
										echo '["'.$row['p_code'].'","'.$descrip.'","'.$row['cantidad'].'","'.$row['descuento'].'","'.$row['precio'].'","'.$sub_total.'"],';
									}
								 ?>
								 [
