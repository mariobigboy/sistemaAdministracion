...
<section class="content-header">
	<div class="content-header-left">
		<h1>Historial de Pagos | Factura Nro: <i id="numFactura"></i></h1>
	</div>
				...
				<div class="box-body table-responsive">
					...
					<table id="tablaClientesCuentas" class="table table-bordered table-striped">
						...
						<tbody>
							...
							$statement = $pdo->prepare("SELECT c.c_id, c.c_apellido, c.c_nombre, c.c_nro_doc, cc.id_factura, cc.tipo, DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(fecha),'+00:00','-03:00'), '%d/%m/%Y %H:%i') fecha_format , cc.deuda, SUM(cc.pago) as pagos FROM cuenta_corriente as cc INNER JOIN tbl_cliente as c ON cc.id_cliente = c.c_id WHERE c.c_id= '$idCliente' GROUP BY cc.id_factura ;");
							$statement->execute();
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="col-12 text-center">
					<strong>Ingresar un Pago:    $</strong><input type="number" name="pago" id="pago" min="0"  placeholder="0"  style="width: 6em;">
				</div>
			</div>
				...
				if (data.length>0) {
					...
					for(var i = 0; i < data.length; i++){
						...
						if (o.pago > 0) {
							...
							linea += '<tr><td>'+i+'</td><td>'+o.fecha_format+'</td><td>'+o.pago+'</td><td>'+saldo+'</td></tr>';
						}
					}
		...
		if (parseFloat($('#saldoRestante').text()) != 0) {
			...
			alertify.confirm('Confirmar pago', 'Va a registrar un pago de $'+pago+' para Factura Nro: '+idFactura+'. ¿Está seguro?', function(){ 
						...
						$.ajax({
							...
							method: 'POST',
							data: [{'name': 'id', 'value' : idFactura }, {'name': 'pago', 'value' : pago }, {'name': 'idCliente', 'value' : idCliente }],
							success: function(data){
