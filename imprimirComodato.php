...
if($orden!=0){
	...
	//obtenemos orden:
	//$statement = $pdo->prepare("SELECT p.*, REPLACE(REPLACE(p.obs,CHAR(10),' '),CHAR(13),' - ') as reemplazo, c.c_apellido, c.c_nombre, DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(p.fecha),'+00:00','-03:00'), '%d/%m/%Y %H:%i') fecha_format FROM `pedido` AS p INNER JOIN factura AS f ON f.num_factura = p.idFactura INNER JOIN tbl_cliente AS c ON c.c_id = f.id_cliente WHERE p.id = ? ;");
	$statement = $pdo->prepare("SELECT 
								...
								tcom.estado estado,
								tcom.orden orden,
								REPLACE(REPLACE(tcom.observaciones,CHAR(10),' '),CHAR(13),' - ') obs,
	...
	foreach($resultado as $row){
		...
		$fecha = $row['fecha_emision_format'];
		$fecha_limite_format = $row['fecha_limite_format'];
		$usuario = $row['user_fullname'];
		$obs = str_replace('-', '\n', $row['obs']);
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var dd = {
				...
				{
					text: '<?php echo "COMODATO" ; ?>',
						//style: 'subheader',
						absolutePosition: {x: 110, y: 43},
						alignment: 'justify',
					},
					{
						text: '<?php echo "Orden de Comodato N°: ".$nro_orden; ?>',
						//style: 'subheader',
						absolutePosition: {x: 330, y: 43},
						bold: true,
						alignment: 'justify',
					},
					{
					...
					/*{
					    ...
					    {
					    	text: 'Entregar en la fecha:', 
					    	style: 'subheader',
					    	absolutePosition: {x: 40, y: 150},
					    },
					    {
					    	text: '<?php echo $fecha_limite_format; ?>',
					    	style: 'valor',
					    	absolutePosition: {x: 175, y: 152},   
					    },
					...
					//aqui va la tabla:
					{
						...
						table: {
							...
							for ( $i = 0; $i < sizeof($resultado); $i++) {
								...
								echo '["'.$cont.'","'.$resultado[$i]['nombre_producto'].'","'.$resultado[$i]['cantidad'].'"],';
								$cont++;
							}
							?>
							[
