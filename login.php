...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['email']) || empty($_POST['password'])) {
        $error_message = 'Email y/o Password no pueden estar vacíos<br>';
    } else {
        ...
        if($total==0) {
            $error_message .= 'Usuario no registrado!<br>';
        } else {       
            foreach($result as $row) { 
                $row_password = $row['password'];
                $row_id = $row['id'];
                $row_full_name = $row['full_name'];
                $row_sucursal_id = $row['sucursal'];
            }
        
            if( $row_password != md5($password) ) {
                $error_message .= 'Password inválido<br>';
            } else {       
