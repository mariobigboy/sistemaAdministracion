...
<section class="content-header">
	...
	<br>
	<?php if ($id==-1) {?>
		<button class="btn btn-info">Venta rapida</button> 
	<?php } ?>
</section>
...
<section class="content">
	...
	<?php if ($id==-1) {?>
				...
				<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
					<div class="form-group">
						...
						<div class="col-sm-6">
							...
							<div id="results"></div> <a href="clientes-add.php" class="btn btn-info" id="btnNuevoCliente">Nuevo Cliente</a>
						</div>
					</div>
				</form>
			</div>
		...
		<div class="box box-info" id="resultados">
			<div class="box-body">
...
<?php }else{ header("location: nuevaVentaCliente.php?id=".$id); ?>
		...
		<div class="modal fade" id="ventaModal" tabindex="-1" role="dialog" aria-labelledby="myLabelModal" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
						<h4 class="modal-title" id="myLabelModal" style="color: #000;"><i class="fa fa-number"></i>Registrar Venta</h4>
					</div>
					<div class="modal-body">
						...
						<div class="box box-info">
							<div class="box-body">
								<form >
									<input type="hidden" id="idUsuario" value="<?php echo $_SESSION['user']['full_name'];?>" class="form-control">
									<input type="hidden" id="idCliente" value="" class="form-control">
									<input type="hidden" id="lista" value="" class="form-control">
									<input type="hidden" id="totalVenta" value="" class="form-control">
										...
										<div class="col-lg-6">
											...
											<div class="col-lg-6">
												...
												<select name="metodo" id="metodo" class="form-control" required>
													...
													<option value="4">A Cuenta</option>
									...
									<div class="form-group">
										...
										<div class="col-lg-6">
											<div class="box box-info">
												<div class="box-body" style="background-color: #f0f0f0" id="tagi">
													<h4>IVA Incl. 21.0%</h4>
													...
													<h4>Total: $ <span id="totalVentaFin"></span></h4>
												</div>
											</div>
		...
		<script type="text/javascript">
			...
			// $('#interesVenta').on('keyup', function(){
			// 	var pago=0;
			// 	if (parseFloat($('#aCobrar').val())>0) {
			// 		pago += parseFloat($('#aCobrar').val());
			// 	}
			// 	if(parseFloat($('#aCobrar1').val())>0){
			// 		pago +=parseFloat($('#aCobrar1').val());
			// 	}

			// 	if(parseFloat($('#aCobrar2').val())>0){
			// 		pago +=parseFloat($('#aCobrar2').val());
			// 	}

			// 	var int = parseFloat($('#interesVenta').val());
			// 	int/=100;
			// 	var subtot=0;
			// 	var sub = parseFloat($('#totalVentaFin').text());
			// 	var intereses = (sub * int);
			// 	subtot = sub + (sub * int);
			// 	$('#totalVentaFin').val(subtot);
			...
			function calcularTodo(){
				...
				$('#totalInteres').text(interes_total);

				$('#totalVentaFin').text(total_total);
			...
			function compruebaTotal(){
				/*if ((parseFloat($('#subt1').val()) + parseFloat($('#subt2').val()) + parseFloat($('#subt3').val())) >= parseFloat($('#idTotalFinal').val())) {
					$('#tagi').css('background-color', '#33FF57');
			...
			$('#aCobrar, #aCobrar1, #aCobrar2, #interesVenta1, #interesVenta2, #interesVenta3, #metodo, #metodo1, #metodo2').on('change', function(){
				calcularTodo();	
			});

			$('#aCobrar').on('keyup',function(){
				/*if ($('#aCobrar').val()>0) {
					var tot=parseFloat($('#aCobrar').val()); //cantidad en el input A Cobrar
					var totalinteres = 0;
					var calculoInteres = 0;
					if ($('#metodo').val()==2) {
						if (parseFloat($('#interesVenta1').val())>0) {
							var interes = parseFloat($('#interesVenta1').val()) / 100;
							calculoInteres = tot + (tot * interes);
							totalinteres= (tot * interes);
							$('#interesTotal1').val(calculoInteres);
							tot = calculoInteres;
						}
					}
					$('#pago1Text').text('A Cobrar: $'+tot);
					var tex= $('#metodo option:selected').text();
					$('#met1').text(tex);
					$('#subt1').val(tot);
					$('#totalInteres').text(totalinteres.toFixed(2));
				}else{
					$('#pago1Text').text("");
					$('#met1').text("");
					$('#subt1').val("0");
			...
			$('#aCobrar1').on('keyup',function(){
				/*if ($('#aCobrar1').val()>0) {
					var tot=$('#aCobrar1').val();
					$('#pago2Text').text('A Cobrar: $'+tot);
					var tex= $('#metodo1 option:selected').text();
					$('#met2').text(tex);
					$('#subt2').val(tot);
				}else{
					$('#pago2Text').text("");
			...
			$('#aCobrar2').on('keyup',function(){
				/*if ($('#aCobrar2').val()>0) {
					var tot=$('#aCobrar2').val();
					$('#pago3Text').text('A Cobrar: $'+tot);
					var tex= $('#metodo2 option:selected').text();
					$('#met3').text(tex);
					$('#subt3').val(tot);
				}else{
					$('#pago3Text').text("");
...
//====================SCAN===============================
	...
	$("#btnAgregarDetalleProducto").on('click', function(){
			...
			if ($("#descrProductPedido").val() == "" || $("#cantProductPedido").val() == "" || $("#precioProductPedido").val() == "") {
				...
				if ($("#tablaProductosVenta tbody tr").length == 0) {
					var linea = '<tr id ="id0" data-id ="0" data-producto="HD0001" data-code="HD0001" data-desc="'+arr[1].value+'" data-cant="'+arr[2].value+'" data.descuento="0" data-prec="'+arr[3].value+'"><th>"HD0001"</th><td>'+arr[1].value+'</td><td>'+arr[2].value+'</td><td>'+arr[3].value+'</td><td></td><td>'+tot.toFixed(2)+'</td></tr>';
					var id_Dom = 'id0';
					var dataID = 0;
				}else{
					var idT = $('table#tablaProductosVenta tbody tr:last').data('id'); 
					var linea = '<tr id ="id'+(idT+1)+'" data-id ="'+(idT+1)+'" data-producto="HD0001" data-code="HD0001" data-desc="'+arr[1].value+'" data-cant="'+arr[2].value+'" data.descuento="0" data-prec="'+arr[3].value+'"><th>'+arr[0].value+'</th><td>'+arr[1].value+'</td><td>'+arr[2].value+'</td><td>'+arr[3].value+'</td><td></td><td>'+tot.toFixed(2)+'</td></tr>';
					var id_Dom = 'id'+(idT+1);
					var dataID = (idT+1);
				}
