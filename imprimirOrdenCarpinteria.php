...
if($id!=0){
		...
		//obtenemos orden:
	$statement = $pdo->prepare("SELECT p.*, REPLACE(REPLACE(p.obs,CHAR(10),' '),CHAR(13),' - ') as reemplazo, c.c_apellido, c.c_nombre, DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(p.fecha),'+00:00','-03:00'), '%d/%m/%Y %H:%i') fecha_format FROM `carpinteria` AS p INNER JOIN factura AS f ON f.num_factura = p.idFactura INNER JOIN tbl_cliente AS c ON c.c_id = f.id_cliente WHERE p.id = ? ;");
