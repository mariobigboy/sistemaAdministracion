...
if($id!=0){
		...
		//obtenemos factura:
	$statement = $pdo->prepare("SELECT t1.*, t2.*, t3.p_name, t3.p_code, t4.*, DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(t2.fecha),'+00:00','-03:00'), '%d/%m/%Y %H:%i') fecha_format, REPLACE(REPLACE(t2.obs,CHAR(10),' '),CHAR(13),' - ') AS reemplazo FROM `detalle` AS t1 INNER JOIN notaCredito AS t2 ON t2.id = t1.nota_credito INNER JOIN factura as fac ON t2.idFactura = fac.`num_factura` INNER JOIN tbl_product AS t3 ON t1.id_producto = t3.p_id INNER JOIN tbl_cliente AS t4 ON fac.id_cliente = t4.c_id WHERE t2.id =?;");
	$statement->execute(array($id));
	$resultado = $statement->fetchAll(PDO::FETCH_ASSOC);
	$total_total = 0;

	foreach($resultado as $row){
		$num_factura = $row['idFactura'];
		$nc = $row['id'];
...
<head>
	<meta charset="UTF-8">
	<title>Generando Nota de Crédito... | HomeDesign </title>
</head>
<body>
			...
			var dd = {
				...
				},
				{
					...
					},
					{
						text: '<?php echo "  | X |"; ?>',
						//style: 'subheader',
						absolutePosition: {x: 300, y: 43},
						bold: true,
						alignment: 'justify',
					},
					{
						text: '<?php echo "NOTA DE CRÉDITO N°: ".$nc; ?>',
					...
					//aqui va la tabla:
					{
						...
						table: {
							...
							foreach($resultado as $row){
								...
								$sub_total = (floatval($row['cantidad']) * floatval($row['precio'])*-1);
										//$total_total += $sub_total;
										//$total_total += $sub_total;
						...
						{
							style: 'observacion', 
							text: "===== DEVOLUCION DE MERCADERIA CORRESPONDIENTE AL DOCUMENTO <?php echo "N°: ".$num_factura; ?>  =====",
							margin:[130,5,0,0],
