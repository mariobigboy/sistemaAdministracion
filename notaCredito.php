...
<section class="content-header">
	<h1>Ventas</h1>
	<br>
	<a href="notaCredito-add.php" class="btn btn-success">Nueva Nota de Crédito</a href="nuevaVenta.php">
</section>
...
<?php
	...
	if($is_user_admin){
		$stat = $pdo->prepare("SELECT * FROM `notaCredito`;");
		$stat->execute();
	}else{
		$stat = $pdo->prepare("SELECT * FROM `notaCredito` WHERE sucursal = ?;");
			...
			<div class="box-body table-responsive">
				<table id="example1" class="table table-bordered table-striped">
					...
					<tbody>
						...
						if($is_user_admin){
							$statement = $pdo->prepare("SELECT t1.*, t3.s_name as suc, DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(t1.fecha),'+00:00','-03:00'), '%d/%m/%Y %H:%i') fecha_format, t2.c_nombre, t2.c_apellido FROM `notaCredito` as t1 INNER JOIN factura as fac ON fac.num_factura= t1.idFactura INNER JOIN tbl_cliente as t2 ON fac.id_cliente = t2.c_id INNER JOIN tbl_sucursales as t3 ON t3.s_id=t1.sucursal WHERE fac.estado=1 ORDER BY fecha DESC;");
							$statement->execute();
						}else{
							$statement = $pdo->prepare("SELECT t1.*, t3.s_name as suc, DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(t1.fecha),'+00:00','-03:00'), '%d/%m/%Y %H:%i') fecha_format, t2.c_nombre, t2.c_apellido FROM `notaCredito` as t1 INNER JOIN factura as fac ON fac.num_factura= t1.idFactura INNER JOIN tbl_cliente as t2 ON fac.id_cliente = t2.c_id INNER JOIN tbl_sucursales as t3 ON t3.s_id=t1.sucursal WHERE fac.estado=1 AND t1.sucursal= ? ORDER BY fecha DESC;");
						...
						foreach ($result as $row) {
							...
							<tr data-id="<?php  echo $row['num_factura']; ?>">
								...
								<td>
									<a href="imprimir_nc.php?id=<?php echo $row['id']; ?>" target="_blank" class="btn btn-warning btn-xs col-lg-12" ><i class="fa fa-file-pdf-o"></i> Nota Crédito </a>						
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<p>Está a punto de eliminar la Nota de Crédito Nro: <i id="nroFactura" style="color: red"></i>. ESTO NO SE PUEDE DESHACER!!!. </p>
...
<script type="text/javascript">
	...
	$('#btnEliminarFactura').on('click', function(){
		...
		$.ajax({
			...
			data: [{name: 'acc', value: 'eliminarNC'},{name:'codFactura', value: idFac}],
			method: 'POST',
			success: function(data){
				if (data.eliminarNC) {
					alertify.success('La nota de crédito ha sido eliminada!');
					setTimeout(function(){
