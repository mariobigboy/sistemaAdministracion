...
if($id!=0){
		...
		//obtenemos orden:
	$statement = $pdo->prepare("SELECT p.*, REPLACE(REPLACE(p.obs,CHAR(10),' '),CHAR(13),' - ') as reemplazo, c.c_apellido, c.c_nombre, FROM_UNIXTIME(p.fecha, '%d/%m/%Y %H:%i') fecha_format FROM `carpinteria` AS p INNER JOIN factura AS f ON f.num_factura = p.idFactura INNER JOIN tbl_cliente AS c ON c.c_id = f.id_cliente WHERE p.id = ? ;");
				...
				<div class="box-body table-responsive">
					...
					<div class="row">
						<div class="col-md-12">
							...
							$statement = $pdo->prepare("SELECT * FROM `imgPedidos` WHERE idCarpinteria = '$id';");
							$statement->execute();
							$resultado = $statement->fetchAll(PDO::FETCH_ASSOC);
				...
				<div class="col-12 text-center">
					<table class="table table-striped" id="tablaPagos">
						...
						<tbody>
							...
							$statement = $pdo->prepare("SELECT estado,FROM_UNIXTIME(fecha, '%d/%m/%Y %H:%i') as fecha_format, usuario, REPLACE(REPLACE(obs,CHAR(10),' '),CHAR(13),' - ') as reemplazo FROM `estadoPedido` WHERE idCarpinteria = '$id' ;");
							$statement->execute();
							$resultado = $statement->fetchAll(PDO::FETCH_ASSOC);
							$filas = $statement->rowCount();
							if ($filas > 0) {
...
<script type="text/javascript">
	...
	$('#btnGuardarEstado').on('click', function(){
		...
		if (estado !="") {
			if(estado==6 && obs==""){
				...
				var datos = [{'name': 'acc' , 'value': 'modificarEstadoCarpinteria'}, {'name': 'idOrden', 'value':  idOt}, {'name' : 'user', 'value' : user}, {'name' : 'estado', 'value' : estado}, {'name' : 'obs', 'value' : obs}];
				$.ajax({
					url: 'apiOrdenes.php',
					method: 'POST',
					data: datos,
	...
	$('#btnVerOrden').on('click', function(){
		var id = $('#idPedido').val();
		var url = "imprimirOrdenCarpinteria.php?id="+id;
	...
	$('#btnAceptarPedido').on('click', function(){
		alertify.confirm('ATENCION!', 'Al confirmar, ésta OT queda abierta y se marca como "EN PRODUCCIÓN". ¿Desea Continuar?', function(){ alertify.warning('Un momento por favor...');
			var idOt = $('#idPedido').val();
			var user = $('#user').val();
			var datos = [{'name': 'acc' , 'value': 'abrirOtCarpinteria'}, {'name': 'idOrden', 'value':  idOt}, {'name' : 'user', 'value' : user}];
			$.ajax({
				url: 'apiOrdenes.php',
				method: 'POST',
				data: datos,
