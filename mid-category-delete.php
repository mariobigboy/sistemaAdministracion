...
<?php
	...
	if(isset($ecat_ids)) {
		...
		for($i=0;$i<count($p_ids);$i++) {
			...
			//not delete, only update and change status:
			$statement = $pdo->prepare("UPDATE tbl_product SET p_is_deleted = ?, p_is_featured = ?, p_is_active = ? WHERE p_id = ?");
			$statement->execute(array(1, 0, 0, $p_ids[$i]));
