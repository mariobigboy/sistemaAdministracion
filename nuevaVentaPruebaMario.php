...
//recibo el id del presupuesto

$statement = $pdo->prepare("SELECT * FROM presupuesto WHERE id_presupuesto = '$id'");
$statement->execute();
...
//<tr id="id0" data-id="0" data-producto="106" data-code="HD1559339129" data-desc="Producto Prueba" data-cant="1" data-descuento="0" data-prec="242"><th>HD1559339129</th> <td>Producto Prueba</td> <td>1</td> <td>$ 242</td> <td>0</td> <td>$ 242.00</td> </tr>
?>
...
<section class="content">
	...
	<input type="hidden" id="ideCliente" value="<?php echo $idCliente; ?>">
	<div class="box box-info" id="membrete">
		<div class="box-body">
			...
			<table class="table " id="tablaProductosVenta" style="background-color: #f0f0f0; box-shadow: 1px 1px 5px #000">
				...
				<tbody>
					<?php 
						$statement = $pdo->prepare("SELECT * FROM detalle WHERE id_presupuesto = '$id'");
						$statement->execute();
						$result = $statement->fetchAll(PDO::FETCH_ASSOC);
						$i=0;
						foreach($result as $row){
							if ($row['id_producto']=='-1') {
								...
								echo "<tr id='id".$i."' data-id='".$i."' data-producto='HD0001' data-code='HD0001' data-desc='".$row['nombre']."' data-cant='".$row['cantidad']."' data-descuento='".$row['descuento']."' data-prec='".$row['precio']."'><th>HD0001</th> <td>".$row['nombre']."</td> <td>".$row['cantidad']."</td> <td>$ ".$row['precio']."</td> <td>".$row['descuento']."</td> <td>$ ".$subtotal."</td> </tr>";
							}else{
								$statement1 = $pdo->prepare("SELECT p_code FROM tbl_product WHERE p_id =?");
								$statement1->execute($row['id_producto']);
								$result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
								$codigoProducto= $result1[0];
								$subtotal = ($row['cantidad']* $row['precio'])+(($row['precio']*$row['descuento'])/100);
								echo "<tr id='id".$i."' data-id='".$i."' data-producto='".$row['id_producto']."' data-code='".$codigoProducto."' data-desc='".$row['nombre']."' data-cant='".$row['cantidad']."' data-descuento='".$row['descuento']."' data-prec='".$row['precio']."'><th>".$codigoProducto."</th> <td>".$row['nombre']."</td> <td>".$row['cantidad']."</td> <td>$ ".$row['precio']."</td> <td>".$row['descuento']."</td> <td>$ ".($row['cantidad']* $row['precio']+(($row['precio']*$row['descuento'])/100))."</td> </tr>";
							}
							$i++;
						}
				...
				<tfoot>
					<th>Total:  $<input type="number" name="idTotalFinal" id="idTotalFinal" value="<?php echo $total; ?>" disabled style="margin-left: 2em;"></th>
				</tfoot>
			...
			<div class="modal-body">
				...
				<form id="formPromo"> 
					...
					<div width="80%">
						...
						<input type="number" name="precioPromo" id="precioPromo" class="form-control" min="0" placeholder="$ 0.00" min="-5000000" max="-1" required>
					</div>
				</form>
